        ...
        <div class="row">
            <div class="col-lg-8 col-md-8 property-detail-inner">
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <h4 class="f-color-red">ราคา: THB <?=number_format($asset->price_amounnt)?></h4>
                    </div>
                    <?php if (!is_null($asset->price_per_wah) && $asset->price_per_wah != '') { ?>
                        <div class="col-md-6"></div>
                        <div class="col-md-6">
                            <h4 class="f-color-red">ราคาต่อตารางวา: THB <?=number_format($asset->price_per_wah)?></h4>
                        </div>
                    <?php } ?>
                     ...
                     <?php if (!is_null($asset->price_rent) && $asset->price_rent != '') { ?>
                        <div class="col-md-6"></div>
                        <div class="col-md-6">
                            <h4 class="f-color-red">ให้เช่า: THB <?=number_format($asset->price_rent)?></h4>
                        </div>
                    <?php } ?>

                    <?php if(!(is_null($asset->youtube_link) || $asset->youtube_link =='')){?>
                        <?php
                        $videoId = '';
                        if (preg_match('/v=([a-zA-Z0-9_-]+)/', $asset->youtube_link, $matches)) {
                                $videoId = $matches[1];
                            }

                            // youtu.be/
                            if (preg_match('/youtu\.be\/([a-zA-Z0-9_-]+)/', $asset->youtube_link, $matches)) {
                               $videoId = $matches[1];
                            }

                            // shorts/
                            if (preg_match('/shorts\/([a-zA-Z0-9_-]+)/', $asset->youtube_link, $matches)) {
                                $videoId = $matches[1];
                            }
                        ?>
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        ...
                        <a href="https://www.google.com/maps/search/?api=1&query=<?=$asset->latitude?>,<?=$asset->longitude?>" target="_blank"><h3 style="text-decoration: underline;">กดเพื่อดูแผนที่</h3></a>
                    </div>
                </div>
                <hr/>
            ...
            <div class="col-md-4">
                <div class="agent-contact-sidebar">
                    <div class="agent-profile-sidebar"> 
                        ...
                        <?= $this->Html->image($image_name, ['alt' => $asset->user->firstname, 'class' => '','title'=>$asset->user->firstname]) ?>
